...
public class BooksService {
	...
	public BookDetailsInfo getBookInfo(int bookId) {
		...
		// JSPに渡すデータを設定する
		String sql = "select * ,case when rent_day is null then '貸し出し可' else '貸し出し中' end from books left outer join rentbooks on books.id = rentbooks.book_id where books.id = "
				+ bookId;
	...
	public BookDetailsInfo newbookInfo() {
		...
		// JSPに渡すデータを設定する
		String sql = "select * ,case when book_id > 0 then '貸し出し中' else '貸し出し可' end from books left outer join rentbooks on books.id = rentbooks.book_id "
	...
	public void registBook(BookDetailsInfo bookInfo) {
		...
		String sql = "INSERT INTO books (title, author, publisher, thumbnail_name,thumbnail_url,publish_date,explation,isbn, reg_date,upd_date) VALUES ('"
				...
				+ "','" + bookInfo.getExplation() + "','" + bookInfo.getIsbn() + "'," + "now()," + "now())";
	 ...
	 * @param bookId
	 */
	public void deleteRentBooks(Integer bookId) {
	...
	public void updateBook(BookDetailsInfo bookInfo) {
		String sql = "UPDATE books SET title ='" + bookInfo.getTitle() + "', author = '" + bookInfo.getAuthor()
				...
				+ "', upd_date = now(), explation ='" + bookInfo.getExplation() + "' Where id = "
				+ bookInfo.getBookId();
	...
	public List<BookInfo> perfectMatchBookList(String search) {
		...
		// TODO 取得したい情報を取得するようにSQLを修正
		List<BookInfo> perfectSearchedBookList = jdbcTemplate
				.query("SELECT * FROM books WHERE title = '" + search + "'", new BookInfoRowMapper());
